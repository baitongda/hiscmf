 ...
 * @param return .xls文件
 * */
function excel2issue()
{
    $arr = [
        'id','问题标题','任务内容','优先级别','当前状态','当前阶段','本阶段预计完成时间','任务类型','提出人员','发布时间','要求完成时间','状态标志','当前阶段负责人','问题关闭状态','关闭任务人员','问题关闭原因','产品名称','项目名称','模块名称'
    ];
    error_reporting(E_ALL);
    date_default_timezone_set('Asia/chongqing');
    $objPHPExcel = new \PHPExcel();
    //import("Org.Util.PHPExcel.Reader.Excel5");
    /*设置excel的属性*/
    $objPHPExcel->getProperties()->setCreator("liyuzhao")//创建人
    ->setLastModifiedBy("liyuzhao")//最后修改人
    ->setKeywords("excel")//关键字
    ->setCategory("issues file");//种类
    ...
    for($j=0;$j<count($arr);$j++){
        $ck = num2alpha($i++) . '1';
        $active->getColumnDimension(num2alpha($i))->setWidth(15);
        $active->setCellValue($ck, $arr[$j]);
        $active->getStyle($ck)->getFont()->setSize(13);
        $active->getStyle($ck)->applyFromArray(array('font' => array ('bold' => true)));
    }
    //查询所有问题数据
    ...
    foreach($buglist as $k=>$v){
        ...
        $num=$b+1;//数据从第二行开始录入
        $v1['renwid'] = $v['renwid'];
        $v1['renwbt'] = strip_tags($v['renwbt']);
        $v1['renwnr'] = strip_tags($v['renwnr']); //过滤掉任务内容的html所有标签
        // 'id','问题标题','任务内容','优先级别','当前状态','当前阶段','本阶段预计完成时间','任务类型','提出人员','发布时间','要求完成时间','状态标志','当前阶段负责人','问题关闭状态','关闭任务人员','问题关闭原因','产品名称','项目名称','模块名称'
        $v1['youxjb'] = Db::name('dl_erjidaima')->where('daimid',$v['youxjb'])->value('daimmc'); //优先级别
        $v1['zhuangt'] = Db::name('dl_erjidaima')->where('daimid',$v['zhuangt'])->value('daimmc');//当前状态
        $v1['jied'] = Db::name('dl_erjidaima')->where('daimid',$v['jied'])->value('daimmc');//当前阶段
        $v1['jiedyjwcsj'] = $v['jiedyjwcsj'];//本阶段预计完成时间
        $v1['renwlx'] = Db::name('dl_erjidaima')->where('daimid',$v['renwlx'])->value('daimmc');//任务类型
        //查询出人员的真实姓名
        $v1['fabry'] = Db::name('admin')->where('admin_id',$v['fabry'])->value('admin_realname');//提出人员
        $v1['fabsj'] = $v['fabsj'];//发布时间
        $v1['yaoqwcsj'] = $v['yaoqwcsj'];//要求完成时间
        $v1['biaoz'] = Db::name('dl_erjidaima')->where('daimid',$v['biaoz'])->value('daimmc');//状态标志
        $v1['jiedfzr'] = Db::name('admin')->where('admin_id',$v['jiedfzr'])->value('admin_realname');//当前阶段负责人
        $v1['rw_state'] = Db::name('dl_erjidaima')->where('daimid',$v['rw_state'])->value('daimmc');
        if($v1['rw_state'] != '关闭问题'){
            $v1['rw_state'] = '问题开启';
        }
        $v1['gbry_id'] = Db::name('admin')->where('admin_id',$v['gbry_id'])->value('admin_realname');//关闭人员
        $v1['beizhu'] = strip_tags($v['beizhu']); //关闭原因
        //查出产品名称
        $v1['cp_name'] = Db::name('dl_chanpin')->where('chanpid',$v['chanp_id'])->value('chanpmc');
        //查出项目名称
        $v1['xm_name'] = Db::name('dl_xiangmu')->where('xiangmid',$v['xiangm_id'])->value('xiangmmc');
        $v1['mk_name'] = $v['mk_name'];
        $a = array_values($v1); //获取数据里面的值
        for($j=0;$j<count($a);$j++){
            $ck = num2alpha($i++) . $num;
            $active->getColumnDimension(num2alpha($i))->setWidth(15);
            $active->setCellValue($ck, $a[$j]);
            $active->getStyle($ck)->getFont()->setSize(12);
        }
        $active->getColumnDimension('B')->setWidth(40);
        $active->getColumnDimension('c')->setWidth(80);
        $active->getColumnDimension('s')->setWidth(80);
        $active->getColumnDimension('v')->setWidth(25);
    }

    // 所有单元格默认高度
    $active->getDefaultRowDimension()->setRowHeight(20);
    // 第一行的默认高度
    ...
    $objPHPExcel->getActiveSheet()->setTitle('云海动力任务问题列表');
    $objPHPExcel->setActiveSheetIndex(0);
    header('Content-Type: application/vnd.ms-excel');
 ...
 *
 * @access public
 * @param string $key 传入一个表要查找的字段，一般为id
 * @param string $id 传入一个id，找出此id的所有有关联的id,为字符
 * @param string $str 传入一个数据库的表名,不需要前缀
 * @param string $sign 传入一个标识，1为找出所有上级的id，2为找出所有下级的id,默认为1
 * @return  string 返回一个路径，例如0-1-4...
 * @author  liyuzhao(562405704@qq.com)
 */
function path($key,$id,$table,$sign='1')
{
    if($sign == '1'){
        $list = Db::name($table)->where($key,$id)->find();
        //判断是否为顶级
        if($list['pid'] == 0){
            $p = '0-'.$list['id'];
            return $p;
        }else{
            $a = path($key,$list['pid'],$table);
            $a = $a.'-'.$list['id'];
            return $a;
        }
    }else if($sign == '2'){
        $list = Db::name($table)->where($key,$id)->find();
        $list1 = Db::name($table)->select();
        $arr = array();
        foreach ($list1 as $k=>$v){
            $p = strpos($v['path'],(string)$list['id']);
            if($p){
                $arr[] = $v['id'];
            }
        }
...
/*
 * @access public
 * @param string $id 传入一个id
 * @return array 返回一个路径数组，如果为顶级返回一个id字符
 * 例子：dl_zb_sys表，传入了一个id，返回了这个id的上级admin_id和本身admin_id的值
 * */
function path1($id)
{
    $data = array();
    ...
    //查出上级路径
    if($list['pid'] != 0){
        //$list1 = Db::name('dl_zb_sys')->where('id',$list['pid'])->find();
        $data[] = path1($list['pid']);
        $data[] = $list['admin_id'];
    }else{
        return $list['admin_id'];
    }
