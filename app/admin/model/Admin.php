...
class Admin extends Model
{
     ...
     * 增加管理员
     */
    public static function add($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$group_id=1,$admin_qq)
    {
        ...
        $sldata=array(
            ...
            'admin_qq'=>$admin_qq,  //此参数后面增加，为了增加用户的qq联系
        );
    ...
    public static function edit($data)
    {
        ...
        if(empty($data['admin_open'])){  //后期修改的
            $admin['admin_open'] = 0;
        }else{
            $admin['admin_open']=$data['admin_open'];
        }
        $admin['admin_qq']=$data['admin_qq'];  //修改qq，后面增加的
        if($data['admin_pwd']){
        ...
        if($rst!==false){
            ...
            if($access){
                if(!empty($data['group_id'])){  //后面修改的，增加一个为空判断
                    //修改
